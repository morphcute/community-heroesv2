            ...
            <div className="relative z-10 flex h-full flex-col items-center justify-between text-center">
              <div className="flex flex-col items-center">
                <div className="rounded-full bg-[linear-gradient(135deg,#fef08a,#f59e0b)] p-[3px]">
                  <div className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-full bg-card text-4xl font-black text-primary sm:h-32 sm:w-32 sm:text-5xl">
                    {user.image ? (
                ...
                <div className="mt-4 font-display text-2xl font-black uppercase tracking-[0.06em] text-foreground sm:mt-6 sm:text-3xl sm:tracking-[0.08em]">{user.rank || "Rookie"}</div>
                <div className="mt-2 text-xs text-muted-foreground sm:text-sm">{user.mlbbId ? `MLBB ID ${user.mlbbId}` : "Set your MLBB ID in profile edit"}</div>
              </div>

              <div className="mt-6 w-full border-t border-border pt-4 text-left">
                <div className="flex items-center justify-between text-[0.62rem] font-bold uppercase tracking-[0.18em] text-muted-foreground font-display">
        ...
        <div className="space-y-6">
          <SurfaceCard>
            <div className="mb-5 flex items-center gap-3">
              ...
              <h2 className="font-display text-xl font-black uppercase tracking-[0.06em] text-foreground sm:text-2xl sm:tracking-[0.08em]">Player Summary</h2>
            </div>
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <ProfileStat label="Tournaments" value={user._count.participations} icon={<Trophy className="h-5 w-5 text-primary" />} />
              <ProfileStat label="Win Rate" value="—" icon={<Zap className="h-5 w-5 text-primary" />} />
              <ProfileStat label="Level" value={`Lvl ${xpData.level}`} icon={<Shield className="h-5 w-5 text-primary" />} />
              <ProfileStat label="Server" value={user.server || "—"} icon={<MapPin className="h-5 w-5 text-primary" />} />
            </div>
          </SurfaceCard>
          ...
          <SurfaceCard>
            <div className="mb-5 flex items-center gap-3">
              ...
              <h2 className="font-display text-xl font-black uppercase tracking-[0.06em] text-foreground sm:text-2xl sm:tracking-[0.08em]">Recent Achievements</h2>
            </div>
            {!user.awards || user.awards.length === 0 ? (
              <div className="rounded-[1.4rem] border border-dashed border-border bg-muted/40 px-6 py-12 text-center text-sm text-muted-foreground">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {user.awards.map((award: any) => {
                  ...
                  return (
                    <div key={award.id} className="flex items-center gap-3.5 rounded-2xl border border-border bg-muted/40 p-4 hover:border-primary/20 transition-all hover:scale-[1.01] duration-300">
                      ...
                      <div className="min-w-0">
                        <div className="font-display text-sm font-black uppercase tracking-wider text-foreground flex items-center gap-1.5">
                          {isDiamonds ? (
                            <>
                            ...
                            <>
                              <span className="text-primary">{award.amount.toLocaleString()}</span>
                              <span className="text-primary/70 text-xs">CASH</span>
                            </>
                          )}
                        </div>
                        <div className="text-xs text-muted-foreground truncate mt-0.5" title={award.tournament?.title || "Tournament award"}>
                          {award.tournament?.title || "Tournament award"}
                        </div>
                        <div className="text-[9px] text-muted-foreground font-bold uppercase tracking-wider mt-1">
          ...
          <SurfaceCard>
            <div className="mb-5 flex items-center gap-3">
              ...
              <h2 className="font-display text-xl font-black uppercase tracking-[0.06em] text-foreground sm:text-2xl sm:tracking-[0.08em]">Match History</h2>
            </div>
            <div className="rounded-[1.4rem] border border-dashed border-border bg-muted/40 px-6 py-12 text-center text-sm text-muted-foreground">
        ...
        <div className="space-y-6">
          <SurfaceCard tone="gold">
            <div className="mb-5 flex items-center gap-3">
              ...
              <h2 className="font-display text-xl font-black uppercase tracking-[0.06em] text-foreground sm:text-2xl sm:tracking-[0.08em]">Contact</h2>
            </div>
          ...
          <SurfaceCard>
            <div className="mb-5 flex items-center gap-3">
              <Shield className="h-5 w-5 text-primary" />
              <h2 className="font-display text-xl font-black uppercase tracking-[0.06em] text-foreground sm:text-2xl sm:tracking-[0.08em]">Game Identity</h2>
            </div>
            <div className="space-y-3">
...
function ProfileStat({ label, value, icon }: { label: string; value: React.ReactNode; icon: React.ReactNode }) {
  return (
    <div className="rounded-[1.2rem] border border-border bg-muted/40 px-4 py-3.5 sm:rounded-[1.4rem] sm:py-4">
      <div className="flex items-center justify-between">
        <span className="text-[0.58rem] font-black uppercase tracking-[0.22em] text-muted-foreground">{label}</span>
        {icon}
      </div>
      <div className="mt-3 font-display text-2xl font-black uppercase tracking-[0.06em] text-foreground sm:mt-4 sm:text-3xl sm:tracking-[0.08em]">{value}</div>
    </div>
  );
}
